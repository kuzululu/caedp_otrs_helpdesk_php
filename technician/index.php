 ...
 <div class="container-fluid">
  <div class="row">
    <div class="col-md-12 col-lg-12">
     <h2 class="fw-bolder fst-italic text-uppercase animate__animated animate__fadeIn animate__infinite infinite">Pending Records</h2>
    </div>
  </div>  
 </div> 
</section>
...
<section class="mt-3">
 <div class="container-fluid">
  ...
  <div class="col-md-4 d-md-flex">
    ...
    <input type="search" id="filterPending" class="form-control resetSearch" placeholder="Filter by Remarks, Unit Status, Services, Status Report, End User"> <button class="resetBtn btn btn-secondary btn-sm" type="button">Reset</button>
  </div> 
</div> 
 </div> 
</section>
...
<section>
 ...
 <div class="row">
  ...
  <div class="table-responsive" id="PendingRecords">
...
?>  	 
  <tr class="text-center align-middle">
    ...
    <td class="align-top"><?= nl2br(str_replace(["\\r\\n","\\n",'\\"'],["\n","\n",'"'], htmlspecialchars($row["status_report"]))) ?></td>
  	<td>
  		<a href="pending_report?id=<?= htmlspecialchars(urlencode(base64_encode($row['id']))) ?>&&technician=<?= urlencode(base64_encode($_SESSION['first_name'] . ' ' . $_SESSION['last_name'])) ?>" class="btn btn-outline-secondary btn-sm">View</a>
  	</td>
  </tr>
<?php
$ctr++;
...
$recordsManager = new IncidentRecords($conn);
$totalRecords = $recordsManager->pendingRecordsCount();

$pageCount = ceil($totalRecords / $recordsPerPage);

$rec = $recordsManager->pendingRecords($page, $recordsPerPage);
