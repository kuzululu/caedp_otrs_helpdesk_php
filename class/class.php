...
class CompleteRecords{
 ...
 public function recordsCount(){
 	...
 	$sql = "SELECT COUNT(*) AS total FROM tbl_edp_logs WHERE status_report <> 'Incident Report' AND trim(status_report) <> '' AND technician='".$_SESSION['user_id']."'";
 	$stmt = $this->conn->query($sql);
 	$row = $stmt->fetch_assoc();
 	$total = $row["total"];
 	return $total;
...
class FilterIncidentReports{
 ...
 public function filter($filter){
 	...
 	$stat_report = "Incident Report";

 	$count_query = "SELECT COUNT(*) AS total FROM tbl_edp_logs WHERE (services LIKE '%$filter%' OR status_report LIKE '%$filter%' OR remarks LIKE '%$filter%') AND status_report='".$stat_report."' AND technician='$technician'";
 	$count_result = $this->conn->query($count_query);
 	$row = $count_result->fetch_assoc();
 	$total_count = $row["total"];

  $sql = "SELECT * FROM tbl_edp_logs WHERE (services LIKE '%$filter%' OR status_report LIKE '%$filter%' OR remarks LIKE '%$filter%') AND status_report = '".$stat_report."' AND technician = '".$technician."'";
 	$get = $this->conn->query($sql);
 	$total = $get->num_rows;
 	$data = "";
    ...
    <td class='align-top'>".htmlspecialchars($row['status_report'])."</td>
  	<td>
  		<a href='incident_report?id=".htmlspecialchars(urlencode(base64_encode($row['id'])))."&&technician=".urlencode(base64_encode($_SESSION['first_name'] . ' ' . $_SESSION['last_name']))."' class='btn btn-outline-secondary btn-sm'>View</a>
  	</td>
  </tr>
 	 ";
...
class FilterRecordsManager{
 ...
 public function filter($filter){
 	...
 	$sql = "SELECT * FROM tbl_edp_logs WHERE services LIKE '%$filter%' OR end_user LIKE '%$filter%' OR status_report LIKE '%$filter%' OR technician LIKE '%$filter%'";
 	$get = $this->conn->query($sql);
 	$total = $get->num_rows;
 	$data = "";
 	...
 	<div class='table-responsive'>
  	...
  	if ($total > 0) {
  		...
  		$data .="
		    ...
		    <td class='align-top'>".htmlspecialchars($row["status_report"])."</td>
		  	<td>
		  		<a href='incident_report?id=".htmlspecialchars(urlencode(base64_encode($row['id'])))."' class='btn btn-outline-secondary btn-sm'>View</a>
		  	</td>
		  </tr>";
  				$ctr++;
  		 }
...
class FilterHistoryRecords{
 ...
 public function filter($filter){
 	...
 	$count_query = "SELECT COUNT(*) AS total FROM tbl_edp_logs_history WHERE services LIKE '%$filter%' OR status_report LIKE '%$filter%' OR remarks LIKE '%$filter%'";
 	$count_result = $this->conn->query($count_query);
 	$row = $count_result->fetch_assoc();
 	$total_count = $row["total"];

 	$sql = "SELECT * FROM tbl_edp_logs_history WHERE services LIKE '%$filter%' OR status_report LIKE '%$filter%' OR remarks LIKE '%$filter%' ORDER BY id DESC";
 	$get = $this->conn->query($sql);
 	$total = $get->num_rows;
 	$data = "";
  	 ...
  	 if ($total > 0) {
  	 	...
  	 	while ($row = $get->fetch_assoc()) {
			   ...
			   <td class='align-top'>".htmlspecialchars($row["update_by"])."</td>
			  	<td>
			  		<a href='details_history?id=".htmlspecialchars(urlencode(base64_encode($row['id'])))."' class='btn btn-outline-secondary btn-sm'>View</a>
			  	</td>
			  </tr>
		 		";
						$ctr++;
...
if (empty($update_remSpecify) && empty($update_statsSpecify)) {
	...
	$sqlInsert = "INSERT INTO tbl_edp_logs_history(ticketId, date_troubleshoot, services, end_user, problem, assessment, unit_status, remarks, status_report, technician) SELECT ticketId, date_troubleshoot, services, end_user, problem, assessment, unit_status, remarks, status_report, technician FROM tbl_edp_logs WHERE id=?";
	$stmtInsert = $this->conn->prepare($sqlInsert);
	// $stmtInsert->bind_param("i", $this->fullName, $id);
	...
	$sqlInsert = "INSERT INTO tbl_edp_logs_history(ticketId, date_troubleshoot, services, end_user, problem, assessment, unit_status, remarks, remarks_specific, status_report, stats_report_specify, technician) SELECT ticketId, date_troubleshoot, services, end_user, problem, assessment, unit_status, remarks, remarks_specific, status_report, stats_report_specify, technician FROM tbl_edp_logs WHERE id=?";
	$stmtInsert = $this->conn->prepare($sqlInsert);
	// $stmtInsert->bind_param("i", $this->fullName, $id);
	$stmtInsert->bind_param("i",$id);
	$stmtInsert->execute();
